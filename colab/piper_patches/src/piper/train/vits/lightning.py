...
class VitsModel(L.LightningModule):
    def __init__(
        ...
        **kwargs,
        ...
        # Full model weights for fine-tuning (no optimizer state).
    ...
    def _compute_generator_loss(self, batch: Batch):
        """Generator-only loss for faster validation (skips discriminator)."""
        x, x_lengths, y, _, spec, spec_lengths, speaker_ids = (
            batch.phoneme_ids,
            batch.phoneme_lengths,
            batch.audios,
        ...
        with autocast(self.device.type, enabled=False):
            ...
            loss_mel = F.l1_loss(y_mel, y_hat_mel) * self.hparams.c_mel
            loss_kl = kl_loss(z_p, logs_q, m_p, logs_p, z_mask) * self.hparams.c_kl
            return loss_mel + loss_dur + loss_kl
    ...
    def validation_step(self, batch: Batch, batch_idx: int):
        val_loss = self._compute_generator_loss(batch)
        self.log("val_loss", val_loss, batch_size=self.batch_size)
        return val_loss

    def on_validation_end(self) -> None:
        # TensorBoard audio synthesis is very slow on Colab; use loss curves instead.
    ...
    def _load_ckpt_state_dict(self, ckpt_path: str) -> dict:
        ckpt = torch.load(ckpt_path, map_location=self.device, weights_only=False)
        return ckpt["state_dict"]

    def _warmstart_vocoder_from_ckpt(self, ckpt_path: str):
        old_sd = self._load_ckpt_state_dict(ckpt_path)
    ...
    def _load_pretrained_from_ckpt(self, ckpt_path: str):
        state_dict = self._load_ckpt_state_dict(ckpt_path)
    ...
    def on_fit_start(self):
        ...
        if self._pretrained_ckpt is not None:
            self._load_pretrained_from_ckpt(self._pretrained_ckpt)
            self._pretrained_ckpt = None
        elif self._vocoder_warmstart_ckpt is not None:
